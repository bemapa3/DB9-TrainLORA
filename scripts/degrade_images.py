...
Degradation types:
  - bicubic_down : Downscale then upscale back via bicubic (matches db9 node)
  - gaussian_blur: Apply Gaussian blur
  - jpeg_artifact: JPEG compression artifacts
  - combined     : All of the above (most realistic)

Usage:
  python scripts/degrade_images.py \\
    --input datasets/processed/img \\
    --output datasets/processed/control \\
    --mode combined --scale-factor 2.0
"""
...
def degrade_bicubic(img: Image.Image, scale_factor: float) -> Image.Image:
    """Downscale then upscale via bicubic — matches db9 node's base_upscale_method."""
    w, h = img.size
...
def degrade_jpeg(img: Image.Image, quality: int = 30) -> Image.Image:
    """Compress to JPEG then decompress — adds compression artifacts."""
    buf = io.BytesIO()
...
def degrade_combined(img: Image.Image, scale_factor: float,
    ...
    """Apply all degradation steps in sequence (most realistic)."""
...
def main():
    ...
    parser.add_argument('--scale-factor', type=float, default=2.0,
                        help='Downscale factor for bicubic_down/combined (default: 2.0, matches db9 node)')
    ...
    parser.add_argument('--jpeg-quality', type=int, default=30,
                        help='JPEG quality for artifacts (lower = worse, default: 30)')
    ...
    for i, img_path in enumerate(images):
        try:
            ...
            # Save with SAME filename (required for pair matching)
            ...
            print(f"[{i+1}/{len(images)}] ✅ {img_path.name} -> {out_path.name} ({args.mode})")
            stats['processed'] += 1
